...
### **Endpoints**
- <Method type="POST">POST</Method> `/v5/generate/song` **- streaming**
...
class V5SongGenRequest(BaseModel):
    ...
    prompt: Optional[str] = Field(None, description="Prompt for song generation")
    reference_url: Optional[str] = Field(
        None, description="URL of the reference audio (if any)"
    )
    instrumental_url: Optional[str] = Field(
        None, description="URL of the instrumental audio (if any)"
    )
    vocal_url: Optional[str] = Field(
        None, description="URL of the vocal audio (if any)"
    )
...
const url = "https://api.soundverse.ai/v5/generate/song";
const headers = {
  Authorization: "Bearer your_api_key_here",
  "Content-Type": "application/json",
};
...
```bash 💻 cURL
curl -X POST "https://api.soundverse.ai/v5/generate/song" \
